...
To do so, get the files from [National Oceanic and Atmospheric Administration - NOAA](https://psl.noaa.gov/) specifically [Files](https://downloads.psl.noaa.gov/Datasets/noaa.oisst.v2/) and here we can work with the two files `sst.wkmean.1990-present.nc` and `lsmask.nc`.
...
Extract the features latitude, longitude, date and sst (sea surface temperature), as well as the `mask`, and do the following tasks:
...
- extract the entire time series for the point (`lat=0.5`, `lon=270.5`), make plots for the following:
   ...
   - plot the weekly average with the minimum and maximum as an _envelope_
...
# | fig-cap: "Weekly average with envelope."
# | code-fold: true
# | echo: false
# | code-summary: "Show the code for the figure"
import pandas as pd
...
## Regression analysis for the time series
...
- Fill the entries by performing the following steps:
   ...
   - the other columns correspond to the weeks, so in row $n$ all entries corresponding to week ($1$ or in our case now $0$) should have an entry $1$ and $0$ otherwise.
     ...
     ```
- reshape our timeseries as well as the smoothed versions into column vectors.
...
- Use the _stochastic gradient descent_ method to solve the same problem; for the algorithm from these notes the following parameters work:
  ```python
  cc = sgd(np.random.random((xx.shape[1], 1)), 
           1e-7, xx, yy, 5_000_000, 400, 1e-9)
  ```
- Use a k-folds cross-validation to find the optimal choice for $n$.
You can use the following code block. What about over-fitting?
  ```{python}
  # | eval: false
...
for i in range(n + 1):
    ...
    print(f"For n = {i} the greatest difference is {error}")
```
...
- Why does the final result not add up to 1?
...
# |      - "Relative error of the projected and reconstructed U component of the PCA."
# | code-fold: true
# | echo: false
# | code-summary: "Show the code for the figure"
%config InlineBackend.figure_formats = ["svg"]
...
print(f"Cumulative explained variance for the first 25 values: {np.cumsum(explained_variance_ratio)[24]}")
```
...
\end{aligned}
$$
Let us do this step by step to get an idea of how this works.

- Define a `pandas` `DataFrame` with our example time series and the date.
- Define a threshold for an acceptable swimming temperature (as we are in a warm climate, we use $25$ in this example to see something).
...
- To see the update progress we update per decades, i.e. $\leq 1999, 2009, 2019, 2029$ (if data exists):
   ...
   - To get a lower and upper bound of the estimate we can use the inverse CDF (PPF) function for $2.5\%$ and $97.5\%$ form `scipy.stats.beta`.
   - Plot `week` vs.  `expected value` with a `plt.fill_between` with the lower and upper bound from above. 


```{python}
# | label: fig-appendices-oe-bayas
# | fig-cap: "Weekly swimmability: Bayesian posterior mean and 95% CI."
# | code-fold: true
# | echo: false
# | code-summary: "Show the code for the figure"
import numpy as np
