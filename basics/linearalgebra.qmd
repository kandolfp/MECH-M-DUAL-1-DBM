...
See [`numpy.testing`](https://numpy.org/doc/stable/reference/routines.testing.html#) for help more ways of testing in `numpy`.
:::
...
So if we have a vector $b$ represented in $\mathcal{B}$ we can compute compute its representation in $\hat{b}$ in $\mathcal{C}$ as
$$
