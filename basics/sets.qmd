...
```
From this Python snippet we know that for tenants the rent variated between `{python} float(nm_min)` and `{python} float(nm_max)`, with an average of `{python} float(nm_mean_r)` and a median of `{python} float(nm_median)`.
...
The surprising answer is `{python} rooms` and it was build in `{python} year` but how do we obtain these?

We can use the `numpy.argwhere` function or some exit directly like `numpy.argmax`.
```{python}
#| classes: styled-output
max_index = np.argmax(data['nm'])
rooms = int(data['rooms'][max_index])
...
Two wildly used packages are `matplotlib` and [`plotly`](https://plotly.com/graphing-libraries/).
:::
...
fig.show()
```
The plot contains the box which is defined by the 1st quantile $Q_1$ and the 3rd $Q_3$, with the median as line in between these two.
Furthermore, we can see the whiskers which help us identify so called outliers.
By default they are defined as $\pm 1.5(Q_3 - Q_1)$, where ($Q_3 - Q_1$) is often called the _interquartile range_.
...
The _spread_ or (_dispersion_, _variability_, _scatter_) are measures used in statistics to classify how data is, well, spread out.
...
With `numpy` the this they are computed as
...
```{python}
#| label: fig-hist
#| fig-cap: "Histogram for the number of rooms in our dataset"
#| code-fold: true
#| code-summary: "Show the code for the figure"
...
Another helpful histogram is @fig-hist2 for the amount of buildings build per year.
```{python}
#| label: fig-hist2
#| fig-cap: "Histogram for the buildings build per year."
#| code-fold: true
#| code-summary: "Show the code for the figure"
index = np.array(range(0, len(data['rooms'])))
...
1. the _year of building_ - `bj` vs. if _central heating_ - `zh0` is available
1. the _year of building_ - `bj` vs. the _city distric_ - `bez`
```{python}
...
fig.update_xaxes(title_text="build in year", row=2, col=1)
...
fig.update_xaxes(title_text="build in year", row=3, col=1)
fig.update_yaxes(title_text="city district", row=3, col=1)

fig.show()
```

In the first plot of @fig-correlation we see that the rent tends to go up with the area of the flat but there are for sure some rather cheap options in terms of space.

The second plot of @fig-correlation tells us that around $1966$ central heating became a stable.
Of course we can also guess that the older buildings with central heating where renovated but we have no data supporting this claim.

The final, third plot of @fig-correlation does not yield an immediate correlation.

More formal we can describe possible correlations via the _covariance_.
The covariance is a measure of the joint variability of tow random variables.

::: {.callout appearance="simple"}
## Definition - covariance
For two finite sets represented by a vectors $v, w\in\mathbb{R}^n$ the **covariance** is defined as
$$
...
In our example, we have rent per square meter which makes some sense.
...
In `numpy` the function `numpy.cov` computes a matrix where the diagonal is the variance the values and the off-diagonals are the covariances of the $i$ and $j$ samples.
...
:::{.callout-tip}
We showed some basic test for correlation, there are more elaborate methods but they are subject to a later chapter.
:::