...
::: {.callout-note}
The computation of the eigendecomposition is not cheep, therefore this is not always worth the effort and there are other ways of solving linear systems.
:::
...
$$ {#eq-hoode}
we can use stack the derivatives into a vector
$$
\begin{array}{ccc}
x_1 & = & x\\
x_2 & = & \dot{x}\\
...
If we consider the free vibrations of a weakly damped system we get the equation
$$
...
::: {.callout-warning}
In most cases inverting the matrix is not recommended but to directly solve the generalized eigenvalue problem, [see @GoluLoan, chap. 7.7] for details. 
:::
...
Let us sort the eigenvalues in $\Lambda$ and let us call $W^T=V^{-1}$ than we can write
$$
...
The this approximation can be used to reduce the storage demand of an image.
...
1. the computation of $V^{-1}$ is nontrivial unless $A$ is symmetric and $V$ becomes unitary ($V{-1} = V^T$).
