...
$$
If all of the eigenvalues are real (in $\mathbb{R}$) we can sort them form largest to smallest
$$
\lambda_n(A) \leq \cdots \leq \lambda_2(A) \leq \lambda_1(A)
$$
we we denote the largest by $\lambda_{max}(A) = \lambda_1(A)$ and the smallest by $\lambda_{min}(A) = \lambda_n(A)$, respectively.
...
If $A \in \mathbb{C}^{n\times n}$ has $n$ (linear) independent eigenvectors $v_1, \ldots, v_n$ and $Av_i=\lambda_i v_i$, for $i=1:n$, than $A$ is _diagonalizable. 
...
\end{array}
$$
As $\Lambda^{-1} = \operatorname{diag}\left(\lambda_1^{-1}, \ldots, \lambda_n^{-1}\right)$ this is easy to compute once we have the singular value decomposition.
:::
