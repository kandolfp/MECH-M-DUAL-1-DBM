...
As $C$ is symmetric and positive semi-definite, therefore it has non-negative real eigenvalues and the matrix $V$ of the eigendecomposition satisfies $V^{-1} = V^T$ (i.e. it is orthogonal @def-orthonormalmatrix).
...
The correlation coefficients $x_r = U_r^T x$ might reveal patterns for different $x$.
...
We define the matrix $A^\dagger \in \mathbb{R}^{m\times n}$ by $A^\dagger = V\Sigma^\dagger U^T$ where
$$
...
[Compare @GoluLoan, p 290]
::::
:::

If we only use the truncated version, i.e. where we only use non zero singular values, we can use it to find good solutions to @eq-eq-leastsquare.
