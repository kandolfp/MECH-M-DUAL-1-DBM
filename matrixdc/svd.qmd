...
![Truncated SVD](../_assets/matrixdc/TruncatedSVD){#fig-tsvd width=2.88in}

We can use this for image compression.

```{python}
# | label: fig-mci_headquater_svd
# | fig-cap: "Image of MCI I and the reconstruction with reduced rank SVD."
# | fig-subcap:
# |      - "Singular values from largest to smallest"
# |      - "0.1% of singularvalues"
# |      - "10% of singularvalues"
# |      - "25% of singularvalues"
# |      - "50% of singularvalues"
# |      - "75% of singularvalues"
# |      - "original image"
...
The main idea of PCA is to transform the data in such a way that the main directions (principal components) capture the largest variation. In short we perform a change of the basis, see @def-changebasis.
...
center = np.array([2, 1])  # Center of data (mean)
...
X = R @ np.diag(sigma) @ np.random.randn(2, nPoints) + np.diag(center) @ np.ones(
    (2, nPoints)
)
...
def my_plot(testFaceMS, avgFace, U, m, n, rec):
    for i, p in enumerate(rec):
        plt.figure()
        r = p
        A_r = avgFace + U[:, :r] @ U[:, :r].T @ testFaceMS
        plt.imshow(np.reshape(A_r, (m, n)).T, cmap=plt.get_cmap("gray"))
        plt.gca().set_axis_off()
        plt.gca().set_title(f"${r = }$")
...
my_plot(testFaceMS, avgFace, U, m, n, rec)

plt.figure()
plt.imshow(np.reshape(testFace, (m, n)).T, cmap=plt.get_cmap("gray"))
plt.gca().set_axis_off()
plt.gca().set_title(f"Original image")

shift = 1500
testFaceMS = (
    np.reshape(im_gray[shift : shift + n, shift : shift + m].T, n * m) - avgFace
)
rec = [100, 400, 1600]
...
plt.show()
```
::: {#fig-eigfaces layout-ncol=4}

![$r = 25$](../_assets/matrixdc/eigenfaces_face_25.svg){#fig-eigfaces1}

![$r = 100$](../_assets/matrixdc/eigenfaces_face_100.svg){#fig-eigfaces2}

![$r = 400$](../_assets/matrixdc/eigenfaces_face_400.svg){#fig-eigfaces3}

![Original image](../_assets/matrixdc/eigenfaces_face_org.svg){#fig-eigfaces4}

![$r = 100$](../_assets/matrixdc/eigenfaces_mci_100.svg){#fig-eigfaces5}

![$r = 400$](../_assets/matrixdc/eigenfaces_mci_400.svg){#fig-eigfaces6}

![$r = 1600$](../_assets/matrixdc/eigenfaces_mci_1600.svg){#fig-eigfaces7}

![Original image](../_assets/matrixdc/eigenfaces_mci_org.svg){#fig-eigfaces8}

Approximate reconstruction of a test face and an object using the eigenfaces basis for different order $r$.
:::
