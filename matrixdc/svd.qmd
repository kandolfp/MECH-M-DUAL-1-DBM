...
::: {.callout-note}
We call $\sigma_i$ a _singular value_ of $A$, the $u_i$ are called the _left singular values_ of $A$ and $v_i$ the _right singular values_ of $A$.
...
If we compute the eigendecompositions of $AA^T$ and $A^TA$.
The two matrices have the same positive eigenvalues - the squares of the eignevalues of $A$ and we get
...
![Singular Value Decomposition](../_assets/matrixdc/SVD.svg){.lightbox}
...
![Thin SVD](../_assets/matrixdc/ThinSVD.svg){.lightbox}
...
If $A \in \mathbb{R}^{m\times n}$ and has the SVD $A = U\Sigma V^T$ than
$$
...
![Truncated SVD](../_assets/matrixdc/TruncatedSVD.svg){.lightbox}
...
1. translation in the direction $\left[2\ 1\right]^T$.
...
As $C$ is symmetric and positiv semi-definite, therefore it has non-negative real eigenvalues and the matrix $V$ of the eigendecompostion satisfies $V^{-1} = V^T$ (i.e. it is orthogonal @def-orthonormalmatrix).
...
::: {.callout-tip}
If the sensor ranges of our matrix are very different in magnitued the correlation matrix is scalled by the _row wise standard deviation_ of $B$ similar as for the mean.
:::
...
The idea is tho apply the PCA techniques to a large set of faces to extract the dominate correlations between the images and create a _face basis_ that can be used to represent an image in these coordinates.
...
We compute the _avarge face_ and subtract it from our dataset to get our matrix $B$.
From here a SVD provides us with our basis $U$.
To test our basis we use individual 37 and a portion of the image of the MCI Headquater (to see how well it performs on objects).
...
::: {.callout-note}
We can understand this as _encoding_ and _decoding_ our test image, which is the general buildup of an _autoencoder_ (a topic for another lecture).

The corrolation coefficients $x_r = U_r^T x$ might reveal patterns for different $x$.
...
```{python}
#| label: fig-eigfaces
#| fig-cap: "Approximate reconstruction of a test face and an object using the eigenfaces basis for different order r."
#| code-fold: true
#| code-summary: "Show the code for the figure"
import numpy as np
import scipy
...
testFaceMS = np.reshape(im_gray[1500:1500+n, 1500:1500+m].T, n*m) - avgFace
rec = [100, 400, 1600]
for i, p in enumerate(rec):
    r = p
    A_r = avgFace + U[:, :r] @ U[:, :r].T @ testFaceMS
...
axs[7].imshow(im_gray[1500:1500+n, 1500:1500+m], cmap=plt.get_cmap("gray"))
axs[7].set_axis_off()
