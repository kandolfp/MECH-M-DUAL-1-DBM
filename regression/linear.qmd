...
plt.show()
```
If we use @eq-fit2, the regression fit is called the _least squares fit_.
...
Polynomial regression, despite its name, is linear regression with a special function $f$ where the relation is polynomial in $x= \left[x_1, \dots, x_m\right]^{\mathsf{T}}$
$$
...
fig2 = go.Figure()
fig.add_trace(
    go.Scatter(mode="markers", x=data["years"], y=data["population"], name="data")
)
fig2.add_trace(
    go.Scatter(mode="markers", x=data["years"], y=data["population"], name="data")
)
