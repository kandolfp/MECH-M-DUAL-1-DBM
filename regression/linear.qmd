...
and
$$
v = (X^TX)^{-1}X^Ty \equiv X^\dagger y.
$$
...
Polynomial regression, despite its name, is linear regression with a special function $f$ where the relation is polynomial in $x= \left[a_1, \dots, x_m\right]^T$
$$
y_k = x_k^0 + x_k^1 b_1 + x_k^2 b_2 + \cdots + x_k^m b_m, \quad 1\leq k \leq n,
$$
...
In the following example we generate an artificial sample of $n=100$ points the samples 
$$
