...
```{python}
#| label: fig-gd-ex1
#| fig-cap: "Gradient decent applied for the function $f(x) = x_1^2 + 3x_2^2$."
#| code-fold: true
...
In order to get a better idea on how this is working for _curve fitting_ we apply the gradient decent method to our curve fitting from @sec-regr-linear-ols.
...
$$
b^{(k+1)} = b^{(k)} - \delta (2 X^mathsf{T} X b^{(k)} - 2 X^mathsf{T} y)
$$
...
#| fig-cap: "Line fit with gradient decent for different number of iterations and learning rate 2e-3."
#| code-fold: true
#| code-summary: "Show the code for the figure"
...
grad = lambda b, X, y: 2 * X.T @ (X @ b - y)
update = lambda b, delta, X, y: b - delta * grad(b, X, y)

def gd(b, delta, X, y, n, stop=1e-10):
    diff = 1
    for _ in range(1, n):
        bnew = update(b, delta, X, y)
        diff = np.linalg.norm(bnew - b)
        b = bnew
        if diff < stop: break
    return b
...
b_10 = gd(b, delta, X, y, 50)
b_20 = gd(b_10, delta, X, y, 50)
b_30 = gd(b_20, delta, X, y, 200)
p4 = np.linalg.pinv(X) @ y

xf = np.arange(0, 11, 0.1)
y1 = np.polyval(b_10, xf)
y2 = np.polyval(b_20, xf)
y3 = np.polyval(b_30, xf)
y4 = np.polyval(p4, xf)
...
We call this method _Stochastic Gradient Decent_ (SGD).
...
#| fig-cap: "Line fit with stochastic gradient decent with 1 or 3 samples and 200 iterations as well as the 3 sample version as initial guess for GD with 100 iterations."
#| code-fold: true
#| code-summary: "Show the code for the figure"
import matplotlib.pyplot as plt
import numpy as np
...
def sgd(b, delta, X, y, n, indices=-1, stop=1e-10):
    if indices == -1:
        indices = X.shape[0]
    diff = 1
    for _ in range(1, n):
...
b_10 = sgd(b, delta, X, y, 200, 1)
b_20 = sgd(b, delta, X, y, 200, 3)
b_30 = sgd(b, delta, X, y, 200, 5)
b_ft = gd(b_20, delta, X, y, 150, -1)
p4 = np.linalg.pinv(X) @ y
