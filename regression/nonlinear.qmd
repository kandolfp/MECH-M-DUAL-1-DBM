...
Since we can have saddle points this is not the sole criterion but a necessary one.
Gradient descent, as the name suggest, uses the gradient as _direction_ in an iterative algorithm to find a minimum.

The idea is basically, if you are lost on a mountain in the fog and you cannot see the path, the fastest and most reliable way that only uses local information is to follow the steepest slope down.
...
fig.update_scenes(
    xaxis_title_text=r"x_1", yaxis_title_text=r"x_2", zaxis_title_text=r"f(x)"
)
...
camera = dict(eye=dict(x=1.7, y=1.3, z=2.2))
...
x = (
    (pd.to_datetime(df["Monat"], format="%B %Y").astype(int) / 10**9 - 1262304000.0)
    / 2678400.0
    * 1
    / 12
)
X = pd.concat(
    [x, x**2, x**3, pd.get_dummies(df["month"], dtype="int")], axis=1
).to_numpy()
cat = 122
...
fig = plt.figure()
plt.bar(
    x=pd.to_datetime(df["Monat"][:12], format="%B %Y").dt.month_name(),
...
::: {.callout-note}
There is also quite a difference between men and women that could be categorized separately.
:::
