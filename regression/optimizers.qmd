...
We speak of an over-determined system if we have more rows than columns, i.e. $A$ is tall and skinny and in general there is no solution to @eq-axb but rather we minimize the error according to a norm, see @ex-regr-linear-ols.
...
b = np.random.rand(m)
x0 = # Optimal 2 norm solution without panaltization as initial start
...
> Among competing hypotheses, the one with the fewest assumptions should be selected, or when you have two competing theories that make exactly the same predictions, the simpler one is the more likely.
- Occam's razor
...
> Take random portions of your data and build a model. Do this $k$ times and average the parameter scores (regression loadings) to produce a cross-validated model. 
> Test the model predictions against withheld (extrapolation) data and evaluate whether the model is actually any good. 
