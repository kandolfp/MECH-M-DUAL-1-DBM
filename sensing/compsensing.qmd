...
s = cosamp(Theta, y, s=10, epsilon=1.0e-10, max_iter=10)
frec = idct(s)
...
# | fig-cap: "Cross-validation mean square error of Lasso fit, the green circle marks the optimal choice, the blue line the mean error of the k-folds and the error bar the maximal and minimal error from the k-folds."
# | code-fold: true
# | code-summary: "Show the code for the figure"
import numpy as np
import matplotlib.pyplot as plt
...
It might not be immediately obvious but applications of this split are video surveillance where the background is represented by $L$ and the foreground objects in $S$, face recognition with the eigenfaces in $L$ and shadows, occlusions (like glasses or masks) are in $S$.
...
model = ps.SSPOR(basis=ps.basis.SVD(n_basis_modes=25), n_sensors=25)
model.fit(X)
