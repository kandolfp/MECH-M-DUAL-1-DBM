...
We discussed the principal component analysis in @sec-matrixdc-pca as an application with the SVD.
...
$$ {#eq-rpca-pcp}
with $\|\cdot\|_{\star}$ is called the _nuclear norm_ consisting of the sum of all singular values and we use it as our proxy for the $\operatorname{rank}$.

@eq-rpca-pcp is called _principal component pursuit_ (PCP) and in @10.1145/1970392.1970395 they show that @eq-rpca-pcp converges to @eq-rpca-org for 

1. $\lambda = 1/\sqrt{\max(n,m)}$ for $X^{m \times n}$ and
1. $L$ is not sparse
...
The following implementation of RPCA can be found in [@Brunton2022, pp. 121] with a similar application to the eigenfaces dataset.
...
@fig-sensing_rpca we can see that RPCA effectively filters out occluded regions and shadows form $X$ in $L$.
The missing part is filled in with the most consistent low-rank feature from the eigenfaces. 

## Sparse Sensor Placement

So far we have looked how to construct a signal with random measurements in a generic basis.
...
We can set _tailored sensors_ for a particular library instead of _random sensors_ in a generic library. 

This can be used to reconstruct faces, or classify signals, see [@Brunton2022, Chapter 3.8] and references within.

To see this in action we use the Python package `PySensors` and the example _Sea Surface Temperature (SST) sensors_ from their documentation, [Online](https://python-sensors.readthedocs.io/en/latest/examples/sea_surface_temperature.html#gallery-examples-sea-surface-temperature-ipynb) accessed on the 28th of November 2024.
...
This is achieved with the `SSPOR` algorithm form @8361090.
...
(Compare [Online](https://python-sensors.readthedocs.io/en/latest/examples/sea_surface_temperature.html#gallery-examples-sea-surface-temperature-ipynb) accessed on the 28th of November 2024)
:::
::::