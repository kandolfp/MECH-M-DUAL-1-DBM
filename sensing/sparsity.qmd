...
If we stream an audio signal or view an image on the web we only need to transfer $s$ and not $x$, saving bandwidth and storage as we go. 
:::
...
fig.show()
```
![Upper left quarter of the MCI I image as a 3D surface.](../_assets/sensing/fft3d.png){#fig-mci_sparsity_fft_compress2 width="80%"}
:::
...
![Illustration to show the fastness of pixel (image) space in comparison to images we can make sense of aka _natural_ images.](../_assets/sensing/pixel_space.svg){#fig-sparsity-pixelspace}
