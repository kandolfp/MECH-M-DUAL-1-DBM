...
Here compressible means we can find a basis that allows for a sparse representation of our signal. 
...
As can be seen in @fig-mci_sparsity_fft_compress2 we can express the clouds with few modes and even the raster of the building seems to fit this model nicely.
...
![Illustration to show the vastness of pixel (image) space in comparison to images we can make sense of aka _natural_ images.](../_assets/sensing/pixel_space){#fig-sparsity-pixelspace width=4.77in}
...
Most of the _dimensions_ of pixel space are only necessary if we want to encode this random-noise images but for a cloud with some mountains and a building we only need a few dimensions and are therefore highly compressible. 