...
A compressible signal $x\in \mathbb{R}^n$ may be written in a sparse vector $s\in \mathbb{R}^n$ with a basis transformation (see @def-changebasis for the formal definition) expressed by the matrix $C\in\mathbb{R}^{n\times n}$ and
$$
x = C s.
$$
...
::::: {.callout-important}
It is important to note that this does not imply that $C$ is sparse.
:::::

If the basis is generic such as the Fourier or Wavelet basis, i.e. we do not need to store the matrix $C$ but can generate it on the spot, we only need to store the $K$ non-zero elements of $s$ to reconstruct the original signal.
::::
:::
...
The downside here is that we need to store $U$ and $V$ (@def-def-svdlowrank) even if we reduce the rank.
...
For an $n \time n$ black and white image there are $2^{n^2}$ possible images.
