...
::: {.callout-tip appearance="simple" collapse="true" icon=false}
## {{< fa pen-to-square >}} Exercise - Self implementation @exm-fourier-fft-den
...
\partial_x f(t) &= -\sin(t) \exp\left(-\frac{t^2}{25}\right) - \frac{2}{25}t f(t)
\end{align}
$$
...
$$
\partial_x f(t_k) \approx \frac{f(t_{k+1}) - f(t_k)}{t_{k+1} - t_k}.
$$
...
$$
\dot{u} = \alpha^2 \partial_x^2 u,
$$
for $u(t, x)$ as the temprature distribution in space ($x$) and time ($t$).
By applying the Fourier transform we get $\mathcal{F}\{u(t,x)\}=\hat{u}(t, x)$ and
$$
\dot{\hat{u}} = - \alpha^2\omega^2\hat{u}.
$$
...
$$
u(t, x) = \mathcal{F}^{-1}\{\hat{u}(t, \omega\} = \mathcal{F}^{-1}\{\mathrm{e}^{-\alpha^2\omega^2 t}\} \ast u(0, x) = \frac{1}{2 \alpha \sqrt{\pi t}} \mathrm{e}^{-\tfrac{x^2}{4\alpha^2t}} \ast u(0, x).
$$
...
::: {.callout-tip appearance="simple" collapse="true" icon=false}
## {{< fa pen-to-square >}} Exercise - Self implementation @exm-fourier-fft-heat
...
$$
f(t) = \cos(2\pi t h(t)), \quad \text{with} h(t) = h_0 + (h_1 - h_0)\frac{t}{3 t_1^2},
$$
...
::: {.callout-note}
There is also the Laplace transform that can be understood as a generalized Fourier transform.
This transform is also heavily used in the transformation of PDEs to simpler ODEs or ODEs into algebraic equations. 

Nevertheless, we do not cover this transform here, for the interested reader we recommend [@Brunton2022, Section 2.5] as it introduces the Laplace transform with the tools we covered here. 
:::
...
We look at these in the next section.