...
1. If $f$ is $L$-periodic it follows that $\forall a\in\mathbb{R}$
$$
...
## Fourier Transform
A function $f\,:\, \mathbb{R} \to \mathbb{R}$ is called _Fourier-transformable_ if
$$
...
\int_{-\infty}^\infty|\hat{f}(\omega)|\, \mathrm{d}\omega \le \infty,$$
...
5. **Drift in frequency**, for $a\in\mathbb{R}$
    $$
     \mathcal{F}\{\mathrm{e}^{\mathrm{i} a t} f(t)\} = \hat{f}(\omega - a).
    $$

6. If $f$ is **even** or **odd**, then $\hat{f}$ is even or odd, respectively.
...
If the function is periodic with $L=N\Delta t$ then the discrete Fourier transform is given as
$$
...
>In 1965, James W. Cooley (IBM) and John W. Tukey (Princeton) developed the so called _fast Fourier transform_ (FFT) that scales with $\mathcal{O}(N \log(N))$, which becomes almost linear for large enough $N$, see @Cooley1965.
...
The DFT computation (without generating the matrix) results in approximately $2\times 10^{11}$ multiplications.
...
::: {.callout-note}
We should note that Cooley and Tukey where not the first to propose a FFT but they provided the formulation used today.
...
If we assume that $N = 2^n$, i.e. a power of $2$, in particular $N=2M$, and $F_N$ denotes the matrix of @eq-dft for dimension $N$ we get have $\hat{f} = F_N f$ and $f = \tfrac1N \overline{F_N} \hat{f}$.
If we further split $f$ in the even and odd indices as 
$$e = [f_0, f_2, \ldots, f_{N-2}]^{\mathsf{T}}\in \mathbb{C}^{M}$$
and 
$$o = [f_1, f_3, \ldots, f_{N-1}]^{\mathsf{T}}\in \mathbb{C}^{M}$$
and with @eq-dft we get
$$
\begin{aligned}
...
\end{aligned}
$$
If we then split $\hat{f}$ in an upper and lower part 
...
df = pd.DataFrame(
    {"Frequency": np.round(freq[filter], 3), "PSD": np.round(PSD_noise[filter], 3)}
)
df
```
...
# | fig-subcap:
# |   - "Computation of the derivative with different methods (Exact and spectral are almost the same)."
...
In @sec-eigen-lode we have already seen how the eigendecomposition can be used to compute the solution of ordinary differential equations by changing the basis and transforming the equation into a basis that can be handled in a straightforward manner.
...
This automatically results in periodic boundary conditions for the following examples, see especially @exm-fourier-fft-advection.
...
# |   - "x-t diagram of the heat equation equation. "
# | layout-ncol: 1
# | code-fold: true
# | code-summary: "Show the code for the figure"
import numpy as np
...
::: {.content-visible when-format="html"}
:::: {.callout-caution appearance="simple" collapse="true" icon=false}
::::: {#exr-fourier-fft-advection-self}

## Self implementation @exm-fourier-fft-advection
...
kappa = # use np.fft.fftfreq to compute the wave numbers
...
def rhsWave(uhat_ri, t, kappa, c):
    uhat = uhat_ri[:N] + (1j) * uhat_ri[N:]
    d_uhat =  #derivative with parameter in fourier space
    d_uhat_ri = np.concatenate((d_uhat.real, d_uhat.imag)).astype("float64")
    return d_uhat_ri
...
We can use FFT to compute some base properties of a ground emitter circuit @fig-fft-gec-1 and show how the signal is propagated through this circuit @fig-fft-gec-2.

![Electric circuit](../_assets/signal/electric_circuit){#fig-fft-gec-1 width=2.28in}
...
# The provided figures were used to create the illustation
if False:
    plt.figure()
    plt.plot(f(t), t)
...
![Signal transition for the electric circuit shown above](../_assets/signal/signals){#fig-fft-gec-2 width=4.88in}

We can see that the signal is boosted depending on the characteristic curve of the diode.
The positive part of the sine is boosted more than the negative part.
If we compute the fast fourier transform, as illustrated with the red discretisation points of the curve on the right hand side, we can interpret the FFT coefficients $c_i$ as follows:
...
1. the effective value from Parseval's Theorem:
   ...
   print(r"$$")
   print(
       r"\overline{i_c} = \sqrt{\sum_{i=0}^{15} |c_i|^2}"
       f"= {np.round(effective_value,3)}"
   )
   print(r"$$")
   ```
1. the total harmonic distortion (how the signal is deformed):
   ...
   print(r"$$")
   print(
       r"THD = \frac{\sqrt{\sum_{i=2}^{14} |c_i|^2}}{\sqrt{\sum_{i=1}^{15} |c_i|^2}}="
       f"{np.round(harmonic_distortion,3)}"
   )
   print(r"$$")
   ```
...
We can see a clear peak at 50Hz, but no information about time is given, whereas in the spectrogram we see how the frequency progresses in time.
...
We cannot attain high resolution simultaneously in both time and frequency domain.
