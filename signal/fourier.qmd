...
In 1965, James W. Cooley (IBM) and John W. Tukey (Princeton) developed the so called _fast Fourier transform_ (FFT) that scales with $\mathcal{O}(N \log(N))$.
 which becomes almost linear for large enough $N$. 
...
Gauss being Gauss did not see this as something important and it did not get published until 1866 in his compiled notes. 
:::
...
::: {.callout-note}
The original FFT paper uses bit flipping and similar techniques to boost performance even more.
