...
8. **Derivative in frequency**
    $$
    \mathcal{L}\{t^n f(t)\} = (-1)^n \partial_s^n F(s)
    $$
...
g = lambda n, t: (
    np.heaviside(t - 2 * n * np.pi, 1) - np.heaviside(t - (2 * n + 1) * np.pi, 1)
)
h = lambda n, t: (
    np.heaviside(t - (2 * n + 1) * np.pi, 1) - np.heaviside(t - (2 * n + 2) * np.pi, 1)
)
...
# | fig-cap: "Impulse response for the second order RC low pass filter"
# | code-fold: true
# | code-summary: "Show the code for the figure"
import numpy as np
import matplotlib.pyplot as plt
