...
$$
\mathrm{e}^{a t} \mathcal{F}\{f(t)\} = F(s - a).
$$

(@) **Derivative in time**
$$
\mathcal{F}\{\partial_t f(t)\} = s F(s) - f(0)
$$
...
and obtain 
$$
X(s) = \frac{1}{(1+s^2)(s^2 + 4)} \sum_{n=0}^\infty \mathrm{-n \pi s}.
$$
...
g = lambda n, t: (heavy_side(t - 2 * n * np.pi) - heavy_side(t - (2 * n + 1) * np.pi))
h = lambda n, t: (heavy_side(t - (2 * n + 1) * np.pi) - heavy_side(t - (2 * n + 2) * np.pi))

xa = lambda t: 1 / 2 * (np.sin(t) + np.abs(np.sin(t)))

fun1 = lambda n, t: (1 / 3 * np.sin(t) - (2 * n + 1) / 6 * np.sin(2 * t)) * g(n, t)
...
RC = 1 
heavy_side = lambda t: np.ones_like(t) * (t>=0)
fun = lambda t: 1 / RC * np.exp(-t / RC) * heavy_side(t)
...
RC = 1 
heavy_side = lambda t: np.ones_like(t) * (t>=0)
fun = lambda t: 1 / RC**2 * t * np.exp(-t / RC) * heavy_side(t)
t = np.linspace(-0.5, 8.5, 1024)
