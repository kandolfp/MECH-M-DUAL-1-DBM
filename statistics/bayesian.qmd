...
(@) A person is known to lie one third the time.
This person throws a die and reports it is a 4 (four). \
\
What is the probability that it is actually a four?

(@) There are 3 urns containing 3 white and 2 black, 2 white and 3 black, as well as 4 white and 1 black ball respectively. Each urn can be chosen with equal probability. \
\
...
f = lambda n, p, x: np.vectorize(lambda x: math.comb(n, x))(x) * np.pow(p, x) * np.pow(1 - p, n - x)
...
cfd = lambda x: (x - a) / (b - a) * np.astype((x >= a) & (x < b) , float) + np.astype((x >= b) , float)
...
f = lambda alpha, beta, x: np.pow(beta, alpha) / math.gamma(alpha) * np.pow(1/x, 1 + alpha) * np.exp(-beta/x)
...
$$
p(\frac{k}{n} | x) = \left(\begin{array}{c} n\\ k\end{array}\right) x^k (1-x)^{n-k}.
$$
