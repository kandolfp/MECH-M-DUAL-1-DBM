...
sigma_integrant = lambda mu, sigma, RaH, sigma_0: p_rah_sigma(mu, sigma, RaH) * p_sigma(sigma, sigma_0)
...
for i, RaH in enumerate(RaH_samples):
    ...
    p_mus_rah_norm = quad(lambda x: mu_integrant(x, sigma, RaH, mu), m_int[0], m_int[-1])[0]
    sigmas_norm = quad(lambda x: sigma_integrant(mu, x, RaH, sigma), s_int[0], s_int[-1])[0]

    mu_old = mu
    mu = quad(lambda x: x * mu_integrant(x, sigma, RaH, mu) / p_mus_rah_norm, m_int[0], m_int[-1])[0]
    sigma = quad(lambda x: x * sigma_integrant(mu_old, x, RaH, sigma) / sigmas_norm, s_int[0], s_int[-1])[0]
