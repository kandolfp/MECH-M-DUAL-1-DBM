...
We call two events $A$ and $B$ independent if the fact that we know $A$ happened does not give us any insight of the probability $B$ or indeed if it happens or vice versa. 
...
\
What is the probability that it is actually a four?[^explanation]
\
...
[^explanation]: The question is vague and it is not clear what exactly the answer of the man is. Stackexchange has an excellent discussion of the question - [Link](https://math.stackexchange.com/questions/1084785/a-man-who-lies-a-fourth-of-the-time-throws-a-die-and-says-it-is-a-six-what-is-t) (accessed 16.12.2025).
## Random variable
...
If the cumulative distribution function is continuous everywhere we get a continuous random variable. 
...
An example could be the height of a person (if we can measure continuously).
In this example, the probability that a person is exactly $185m$ is zero, but the question if somebodies hight is in $[180, 190)$ makes sense.
...
It is instructive to look at some of the most common probability distributions so that we know what we can describe by them. 
...
f = (
    lambda n, p, x: np.vectorize(lambda x: math.comb(n, x))(x)
    * np.pow(p, x)
    * np.pow(1 - p, n - x)
)
...
cfd = lambda x: (x - a) / (b - a) * np.astype((x >= a) & (x < b), float) + np.astype(
    (x >= b), float
)
...
plt.figure()
plt.plot(x, f(0, 1, x), label=r"$\mu=0$, $\sigma^2=1$")
plt.plot(x, f(0, np.sqrt(0.1), x), label=r"$\mu=0$, $\sigma^2=0.1$")
plt.plot(x, f(1 / 2, np.sqrt(0.5), x), label=r"$\mu=\frac{1}{2}$, $\sigma^2=0.5$")
plt.gca().set_aspect(6 / (3 * 1.25))
plt.legend()
plt.figure()
plt.plot(x, cfd(0, 1, x), label=r"$\mu=0$, $\sigma^2=1$")
plt.plot(x, cfd(0, np.sqrt(0.1), x), label=r"$\mu=0$, $\sigma^2=0.1$")
plt.plot(x, cfd(1 / 2, np.sqrt(0.5), x), label=r"$\mu=\frac{1}{2}$, $\sigma^2=0.5$")
...
f = (
    lambda alpha, beta, x: np.pow(beta, alpha)
    / math.gamma(alpha)
    * np.pow(1 / x, 1 + alpha)
    * np.exp(-beta / x)
)
...
1. The denominator is there to normalize the posterior so that the posterior becomes a probability distribution with integral 1.

1. The posterior is our main result of the so called Bayers inference. If we have multiple samples we can use it as the prior for the next.
...
For this example we assume no bias.
:::

The probability that exactly $k$ people of a random sample of size $n$ vote for party $x$ is best described with a binomial distribution
$$
...
bino = lambda k, n, x: scipy.special.comb(n, k) * np.pow(x, k) * np.pow(1 - x, n - k)
...
::: {.callout-important}
This example is an adaptation form the notes @mehrle. 
:::

To identify material properties such as strength we can also use Bayes' theorem.

Again we start with an initial guess, in this case for the yield stress with $90\%$ of the samples endure in our experiment so far. 
...
Finv = lambda mu, sigma, p: mu + sigma * np.sqrt(2) * scipy.special.erfinv(2 * p - 1)
...
The likelihood of drawing a sample with yield strength $R_{aH}^{(i)}$ we calculate by the dependent probabilities separately.
To do so we fix the _current_ values of $\mu = \check{\mu}$ and $\sigma = \check{\sigma}$, respectively.
$$
...
Now our updated values for $\mu$ and $\sigma$ are the expected values 
$$
\mathrm{E}[\mu] = \int \mu \cdot p\left( \mu \, \middle|\, R_{aH}^{(i)}\right)
$$
and
$$
\mathrm{E}[\sigma] = \int \sigma \cdot p\left( \sigma \, \middle|\, R_{aH}^{(i)}\right)
$$

We can continue this for several samples.
As we do not have real measurements for this example at hand we generate some sampled from a normal distribution that seems fitting for a _S235JR_ steal.
...
# | code-summary: "Show the code for the figure"
np.random.seed(6020)
RaH_samples = np.random.normal(245, 15, 100)

p_mu = lambda x, mu_0: normaldist(mu_0, mu_0 / 10, x)
p_sigma = lambda x, sigma_0: normaldist(sigma_0, sigma_0 / 10, x)

p_rah_mu = lambda mu, sigma, x: normaldist(mu, sigma, x)
p_rah_sigma = lambda mu, sigma, x: normaldist(mu, sigma, x)

mu_integrant = lambda mu, sigma, RaH, mu_0: p_rah_mu(mu, sigma, RaH) * p_mu(mu, mu_0)
sigma_integrant = lambda mu, sigma, RaH, sigma_0: p_rah_sigma(mu, sigma, RaH) * p_sigma(
    sigma, sigma_0
)
...
sigma = sigma_0
plt.figure()
plt.plot(x, normaldist(mu, sigma, x), label="initial guess")

for i, RaH in enumerate(RaH_samples):
    m_int = [mu - 0.3 * mu, mu + 0.3 * mu]
    s_int = [sigma - 0.3 * sigma, sigma + 0.3 * sigma]

    p_mus_rah_norm = quad(
        lambda x: mu_integrant(x, sigma, RaH, mu), m_int[0], m_int[-1]
    )[0]
    sigmas_norm = quad(
        lambda x: sigma_integrant(mu, x, RaH, sigma), s_int[0], s_int[-1]
    )[0]

    mu_old = mu
    mu = quad(
        lambda x: x * mu_integrant(x, sigma, RaH, mu) / p_mus_rah_norm,
        m_int[0],
        m_int[-1],
    )[0]
    sigma = quad(
        lambda x: x * sigma_integrant(mu_old, x, RaH, sigma) / sigmas_norm,
        s_int[0],
        s_int[-1],
    )[0]

    if (i == 9) or (i == 99):
        plt.plot(x, normaldist(mu, sigma, x), label=f"update after {i+1} iterations")

plt.gca().set_aspect(400 / (3 * 0.022))
plt.grid("major")
plt.legend()
plt.show()
```

The deviation ($\sigma$) converges steadily, whereas the mean $\mu$ is more erratic and only slowly converging.
This is due to the fact that the two variables are not independent of each other and the assumption of a normal distribution is not a good estimate for $\sigma$.
