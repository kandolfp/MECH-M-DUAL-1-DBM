...
for 
$$
K_k = P_{k|k-1} H_k^{\mathsf{T}}(H_kP_{k|k-1}H_k^{\mathsf{T}} + R_k)^{-1}.
$$
...
We can see a possible outcome in @fig-kalman-train1-2 where the train has moved in positive direction but the variance has increased indicating the increased uncertainty in the position due to the fact that we do not have information from the acceleration or deceleration happening in the span $\Delta t$.
...
update = lambda mu, sigma: (
    (mu[0] * np.pow(sigma[1], 2) + mu[1] * np.pow(sigma[0], 2))
