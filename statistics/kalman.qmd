...
and
$$
P_{k|k-1} = F_k P_{k-1|k-1} F_k^{\mathrm{T}}+ Q_k.
$$
...
$$
P_{k|k-1} = E\left[(x_k - x_{k|k-1})(x_k - x_{k|k-1})^{\mathrm{T}}\right].
$$
...
for 
$$
K_k = P_{k|k} H_k^{\mathrm{T}}(H_kP_{k|-1}H_k^{\mathrm{T}} + R_k)^{-1}.
$$
...
plt.gca().set_aspect(5 / (3 * 5))
plt.ylim([0, 5])
plt.legend()
plt.figure()
plt.plot(x, f(1.8, 0.15, x), label=r"t=1")
plt.gca().set_aspect(5 / (3 * 5))
plt.legend()
plt.ylim([0, 5])
plt.show()
...
update = lambda mu, sigma: ((mu[0] * np.pow(sigma[1], 2) + mu[1] * np.pow(sigma[0], 2)) / (np.sum(np.pow(sigma, 2))), np.prod(sigma) / np.sqrt(np.sum(np.pow(sigma, 2))))

x = np.linspace(0, 5, 1024)
plt.figure()
plt.plot(x, f(1.8, 0.2, x), label=r"predictor")
plt.plot(x, f(2, 0.1, x), label=r"corrector")
#plt.plot(x, (f(1.8, 0.2, x) * f(2, 0.1, x)), label=r"updated position")
plt.plot(x, f(*update([1.8, 2.0], [0.2, 0.1]), x), label=r"updated position")
plt.gca().set_aspect(5 / (3 * 5))
plt.legend()
plt.ylim([0, 5])
plt.show()
